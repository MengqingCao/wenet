# Copyright (c) 2024,HW (sxc19@mails.tsinghua.edu.cn)
#
...
def export_encoder(asr_model, args):
    ...
    ort_session = onnxruntime.InferenceSession(
        encoder_outpath, providers=['CANNExecutionProvider'])
...
def export_ctc(asr_model, args):
    ...
    ort_session = onnxruntime.InferenceSession(
        ctc_outpath, providers=['CANNExecutionProvider'])
...
def export_decoder(asr_model, args):
    ...
    torch_score, torch_r_score = decoder(hyps, hyps_lens, encoder_out,
                                         args['reverse_weight'])
    ort_session = onnxruntime.InferenceSession(
        decoder_outpath, providers=['CANNExecutionProvider'])
